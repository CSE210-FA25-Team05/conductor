...
module.exports = async function courseRoutes(fastify, options) {
    ...
    fastify.get('/courses/:course_id', async (request, reply) => {
        try {
            const res = await courseRepo.getCourseById(fastify, parseInt(request.params.course_id, 10));
            return res;
        } catch (error) {
            return mapAndReply(error, reply);
        }
    ...
    fastify.get('/courses/:course_id/users', async (request, reply) => {
        try {
            const res = await courseRepo.getUsersInCourse(fastify, parseInt(request.params.course_id, 10));
            return res;
        } catch (error) {
            return mapAndReply(error, reply);
        }
    ...
    fastify.get('/courses/:course_id/users/:user_id', async (request, reply) => {
        try {
            const res = await courseRepo.getUserDetailsInCourse(fastify, parseInt(request.params.course_id, 10), parseInt(request.params.user_id, 10));
            return res;
        } catch (error) {
            return mapAndReply(error, reply);
        }
    ...
    fastify.patch('/courses/:course_id', async (request, reply) => {;
        try {
            await courseRepo.updateCourse(fastify, parseInt(request.params.course_id, 10), request.body);
            reply.send();
        } catch (error) {
            return mapAndReply(error, reply);
        }
    ...
    fastify.delete('/courses/:course_id', async (request, reply) => {
        try {
            await courseRepo.deleteCourse(fastify, parseInt(request.params.course_id, 10));
            reply.code(204).send();
        } catch (error) {
            return mapAndReply(error, reply);
        }
